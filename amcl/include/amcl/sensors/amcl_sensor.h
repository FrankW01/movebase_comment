...
// Base class for all AMCL sensors   amcl的传感器模型的虚类
class AMCLSensor
{
  // Default constructor
  public: AMCLSensor();
  ...
  // Flag is true if this is the action sensor  如果这是动作传感器，则标志为真
  public: bool is_action;

  // Action pose (action sensors only)  动作姿势（仅限动作传感器）
...
// Base class for all AMCL sensor measurements   测量模型的虚类
class AMCLSensorData
{
