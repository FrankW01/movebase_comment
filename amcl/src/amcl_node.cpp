...
static double
angle_diff(double a, double b) // 计算两角差值，弧度值
{
  double d1, d2;
  a = normalize(a);
  b = normalize(b);
...
class AmclNode
{
  ...
  private:
    ...
    TransformListenerWrapper* tf_;//该类型方法实际调用TransformListener的transformPose方法
    ...
    // key pf entry   最为核心的一个函数  实现了收到观测数据就调用pf等函数进行粒子滤波的权重更新，输出pose的估计，粒子重采样等。
   ...
   //use this to subcribe map topic once
    ...
    // Helper to get odometric pose from transform system 从变换系统获取里程计姿势的助手
    ...
    std::string base_frame_id_;//参数默认是base_link
    ...
    double d_thresh_, a_thresh_;//最小更新里程的距离阈值与角度阈值
    int resample_interval_;默认为2
    int resample_count_;
    double laser_min_range_;
    double laser_max_range_;//默认是-1,
    ...
    ros::Duration transform_tolerance_;//发布map——>odom的时间间隔
...
std::vector<std::pair<int,int> > AmclNode::free_space_indices;  空闲空间cell
        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("laser_min_range", laser_min_range_, -1.0);//要考虑的最小扫描范围; -1.0将导致使用激光器报告的最小范围
  private_nh_.param("laser_max_range", laser_max_range_, -1.0);//要考虑的最大扫描范围; -1.0将导致使用激光器报告的最大范围
  private_nh_.param("laser_max_beams", max_beams_, 30);//在更新过滤器时要在每次扫描中使用多少均匀间隔的光束，什么意思？
  ...
  private_nh_.param("laser_model_type", tmp_model_type, std::string("likelihood_field"));//激光模型
...

  {
    ...
    private_nh_.param("bag_scan_period", bag_scan_period, -1.0);//时间周期默认是-1.0 ，-1有什么特殊功能吗
    bag_scan_period_.fromSec(bag_scan_period);
  }

  updatePoseFromServer();//从参数服务器中加载上次的位姿信息

  cloud_pub_interval.fromSec(1.0);
  tfb_ = new tf::TransformBroadcaster();
  tf_ = new TransformListenerWrapper();

  pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 2, true);//消息队列大小2
  particlecloud_pub_ = nh_.advertise<geometry_msgs::PoseArray>("particlecloud", 2, true);
  global_loc_srv_ = nh_.advertiseService("global_localization", 
					 &AmclNode::globalLocalizationCallback,
                                         this);//全局定位服务
  nomotion_update_srv_= nh_.advertiseService("request_nomotion_update", &AmclNode::nomotionUpdateCallback, this);//nomotion是指什么？
  set_map_srv_= nh_.advertiseService("set_map", &AmclNode::setMapCallback, this);

  laser_scan_sub_ = new message_filters::Subscriber<sensor_msgs::LaserScan>(nh_, scan_topic_, 100);//message_filters::Subscriber<sensor_msgs::LaserScan>* laser_scan_sub_;
  laser_scan_filter_ = 
          new tf::MessageFilter<sensor_msgs::LaserScan>(*laser_scan_sub_, 
                                                        *tf_, 
                                                        odom_frame_id_, 
                                                        100);//tf::MessageFilter<sensor_msgs::LaserScan>* laser_scan_filter_; 
  laser_scan_filter_->registerCallback(boost::bind(&AmclNode::laserReceived,
                                                   this, _1));
  initial_pose_sub_ = nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);

  if(use_map_topic_) {//参数默认是false
  ...
  dsrv_ = new dynamic_reconfigure::Server<amcl::AMCLConfig>(ros::NodeHandle("~"));//动态参数设置
  dynamic_reconfigure::Server<amcl::AMCLConfig>::CallbackType cb = boost::bind(&AmclNode::reconfigureCB, this, _1, _2);
  dsrv_->setCallback(cb);

  // 15s timer to warn on lack of receipt of laser scans, #5209
  laser_check_interval_ = ros::Duration(15.0);//ros::Duration
  check_laser_timer_ = nh_.createTimer(laser_check_interval_, 
                                       boost::bind(&AmclNode::checkLaserReceived, this, _1));//定时触发回调函数
}

void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)//配置参数函数
{
...
void AmclNode::savePoseToServer()//把最后一次的位置保存在initial_pose参数中
{
  ...
  double yaw,pitch,roll;
  map_pose.getBasis().getEulerYPR(yaw, pitch, roll);//getRotation()或getBasis()用于获取旋转矩阵
...
void AmclNode::updatePoseFromServer()//从参数服务器中加载上次的位姿信息
{
  init_pose_[0] = 0.0;
  init_pose_[1] = 0.0;
  init_pose_[2] = 0.0;
...
void 
AmclNode::checkLaserReceived(const ros::TimerEvent& event)//这个形式参数设置为空不可以吗
{
  ros::Duration d = ros::Time::now() - last_laser_received_ts_;//last_laser_received_ts_存储着最新的laser数据的时间
...
void
AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)//根据地图信息创造了地图空闲区域、粒子滤波器、里程计运动模型与激光感知的似然域模型
{
  boost::recursive_mutex::scoped_lock cfl(configuration_mutex_);
  ...
  map_ = convertMap(msg);  //这里将av_msgs::OccupancyGrid 转化为 map_t格式
  ...
  for(int i = 0; i < map_->size_x; i++)
    for(int j = 0; j < map_->size_y; j++)
      if(map_->cells[MAP_INDEX(map_,i,j)].occ_state == -1)//-1 为空闲状态
        free_space_indices.push_back(std::make_pair(i,j));
#endif
  // Create the particle filter  初始化一个粒子滤波器
  pf_ = pf_alloc(min_particles_, max_particles_,
                 alpha_slow_, alpha_fast_,
                 (pf_init_model_fn_t)AmclNode::uniformPoseGenerator,
                 (void *)map_);
  ...
  // Initialize the filter  这个filter与上边那个particles filter有什么区别？    //这是从参数服务器获取初始位姿信息
  ...
  // Instantiate the sensor objects     //定义里程计与激光雷达并初始化
  // Odometry
  delete odom_;
  odom_ = new AMCLOdom();
  ROS_ASSERT(odom_);//ROS_ASSERT:asserts that the provided expression evaluates to true.  If it is false, program execution will abort
  odom_->SetModel( odom_model_type_, alpha1_, alpha2_, alpha3_, alpha4_, alpha5_ );
  // Laser
  delete laser_;
  laser_ = new AMCLLaser(max_beams_, map_);
                      ...
                      const ros::Time& t, const std::string& f)
{
  ...
  try
  {
    this->tf_->transformPose(odom_frame_id_, ident, odom_pose);//获取base_link到odom的变换
  }
...
pf_vector_t
AmclNode::uniformPoseGenerator(void* arg)//在循环中直到产生一个位置与方向都合法的粒子，退出循环。也就是说，每次调用该函数产生一个随机位姿的粒子。     
{
  map_t* map = (map_t*)arg;
#if NEW_UNIFORM_SAMPLING  //随机生成一个空闲区域的粒子
  unsigned int rand_index = drand48() * free_space_indices.size();  //drand48返回服从均匀分布的·[0.0, 1.0) 之间的 double 型随机数
...
AmclNode::globalLocalizationCallback(std_srvs::Empty::Request& req,
                                     std_srvs::Empty::Response& res)//全局定位的服务调用函数
{
  if( map_ == NULL ) {
    return true;
  }
...
// force nomotion updates (amcl updating without requiring motion)   每看懂这个服务的用处，强制更新filter中的odom
bool 
...
void
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)//core function!!!-------------------------------------------------------
{
  last_laser_received_ts_ = ros::Time::now(); //for check laser received重置一个最新的雷达数据
  if( map_ == NULL ) {
    return;
  }
  ...
  if(frame_to_laser_.find(laser_scan->header.frame_id) == frame_to_laser_.end())//can't find, set up lasers_ lasers_update_      //std::map< std::string, int > frame_to_laser_;
  {
    ...
    tf::Stamped<tf::Pose> ident (tf::Transform(tf::createIdentityQuaternion(),
                                             tf::Vector3(0,0,0)),
                                 ros::Time(), laser_scan->header.frame_id);//是laser_scan的坐标系，000
    tf::Stamped<tf::Pose> laser_pose;
    try
    { // transform laser_frame ident to base_frame, saved in laser_pose
      this->tf_->transformPose(base_frame_id_, ident, laser_pose);//将laser_frame的坐标转换为base_link的坐标之下
    }
    catch(tf::TransformException& e)
    {
    ...
    // laser mounting angle gets computed later -> set to 0 here!  稍后计算激光安装角度 -> 此处设置为 0！
    laser_pose_v.v[2] = 0;
    lasers_[laser_index]->SetLaserPose(laser_pose_v);//set laser to base transform   //std::vector< AMCLLaser* > lasers_;设置激光雷达的距离base_link的offset
    ...
    frame_to_laser_[laser_scan->header.frame_id] = laser_index;//record laser_scan's frame, mapped to an index
                                                              // for which lasers stores an entity     记录 laser_scan 的帧，映射到激光存储实体的索引
    //don't care creating or replacing 也就是这里暗含着可以使用多个激光雷达
  } else {
  ...
  if(pf_init_) 
  {
    ...
    delta.v[0] = pose.v[0] - pf_odom_pose_.v[0];//pf_odom_pose_ 是上一次里程计更新位姿
    delta.v[1] = pose.v[1] - pf_odom_pose_.v[1];
    delta.v[2] = angle_diff(pose.v[2], pf_odom_pose_.v[2]);

    // See if we should update the filter  是否超过更新阈值来需要更新滤波器
  ...
  if(!pf_init_) // set pf_init_ false will enter here  首次初始化
  {
    ...
    for(unsigned int i=0; i < lasers_update_.size(); i++)//std::vector< bool > lasers_update_;
      lasers_update_[i] = true;

    force_publication = true;//首次初始化会强制更新一次粒子集权重
  ...
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    odom_->UpdateAction(pf_, (AMCLSensorData*)&odata);//使用里程计数据来更新粒子群位姿
  ...
  if(lasers_update_[laser_index])
  {
    ...
    // min, max, and increment angles of the laser in the base frame.//激光扫描的最小值与角度增量
    //
    // Construct min and max angles of laser, in the base_link frame.  为了考虑倒置安装的激光器，我们确定了基本框架中激光器的最小、最大和增量角度。在 base_link 框架中构造激光的最小和最大角度。
    tf::Quaternion q;
    q.setRPY(0.0, 0.0, laser_scan->angle_min);
    tf::Stamped<tf::Quaternion> min_q(q, laser_scan->header.stamp,
                                      laser_scan->header.frame_id);
    q.setRPY(0.0, 0.0, laser_scan->angle_min + laser_scan->angle_increment);//最小角度+角度增量
    ...
    // wrapping angle to [-pi .. pi] //fmod函数是用来取余数
    angle_increment = fmod(angle_increment + 5*M_PI, 2*M_PI) - M_PI;

    ROS_DEBUG("Laser %d angles in base frame: min: %.3f inc: %.3f", laser_index, angle_min, angle_increment);

    // Apply range min/max thresholds, if the user supplied them  如果用户提供了范围最小/最大阈值，则应用它们  给range_max赋值
    ...
    if(laser_min_range_ > 0.0)
      range_min = std::max(laser_scan->range_min, (float)laser_min_range_);//laser_scan的range_min的参数是什么意思？
    else
    ...
    for(int i=0;i<ldata.range_count;i++)
    {
      ...
      // readings to max range. // amcl（还）没有最小范围的概念。 所以我们将短读数映射到最大范围。 什么意思
      if(laser_scan->ranges[i] <= range_min)
        ldata.ranges[i][0] = ldata.range_max;
      else
        ldata.ranges[i][0] = laser_scan->ranges[i];
      // Compute bearing  计算方位
    ...
    if(!(++resample_count_ % resample_interval_))//resample_count_ = 0  resample_interval_ 
    {
      pf_update_resample(pf_);
      resampled = true; //每次重采样之后，需要对粒子集权重重新计算
    }
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      // subtracting base to odom from map to base and send map to odom instead    从 map 到 base 减去 base 到 odom，然后将 map 发送到 odom
      ...
      try
      {
        tf::Transform tmp_tf(tf::createQuaternionFromYaw(hyps[max_weight_hyp].pf_pose_mean.v[2]),
                             tf::Vector3(hyps[max_weight_hyp].pf_pose_mean.v[0],
                                         hyps[max_weight_hyp].pf_pose_mean.v[1],
                                         0.0));//获得base_link相对于map的坐标
        tf::Stamped<tf::Pose> tmp_tf_stamped (tmp_tf.inverse(),
                                              laser_scan->header.stamp,
                                              base_frame_id_);//tmp_tf_stamped 是base_link的坐标系，储存的是最大权重粒子的点的map坐标的逆
