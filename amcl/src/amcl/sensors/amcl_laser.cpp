...
////////////////////////////////////////////////////////////////////////////////
// Apply the laser sensor model  通过特定的感知模型进行粒子滤波器的更新
bool AMCLLaser::UpdateSensor(pf_t *pf, AMCLSensorData *data)
{
  ...
  else if(this->model_type == LASER_MODEL_LIKELIHOOD_FIELD)//默认更新模型
...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)//似然域测量模型的具体实现
{
  AMCLLaser *self;
  int i, j, step;
  double z, pz;
  ...
  {
    sample = set->samples + j;
    pose = sample->pose;//这是在里程计运动模型更新后的位姿

    // Take account of the laser pose relative to the robot        //这个函数是将传感器局部坐标经过三角变换映射到全局坐标系下
    pose = pf_vector_coord_add(self->laser_pose, pose);

    p = 1.0;

    // Pre-compute a couple of things      //预计算似然域，离散栅格化
    ...
    for (i = 0; i < data->range_count; i += step)
    {
      ...
      // This model ignores max range readings        //似然域测量模型简单地将最大距离读数丢弃
      if(obs_range >= data->range_max)
        continue;
      ...
      // Compute the endpoint of the beam  //计算波束的最终距离并将其转换到全局坐标系下
      ...
      // Part 1: Get distance from the hit to closest obstacle.  //这一块的参数在map_update_cspace函数中计算，具体来说是将整个地图代入并计算障碍物位置，并将最近距离预先计算好，再把当前的栅格代入即可
      // Off-map penalized as max distance
      if(!MAP_VALID(self->map, mi, mj))
        z = self->map->max_occ_dist;
      else
        z = self->map->cells[MAP_INDEX(self->map,mi,mj)].occ_dist;
      // Gaussian model        //将正态分布与均匀分布混合后得到的似然结果
