...
class CachedDistanceMap//这是一个距离模板，类似根据x，y的坐标来直接获取半径大小的查找表
{
  public:
...
{
  if(marked[MAP_INDEX(map, i, j)])//如果之前访问过，就直接弹出
    return;

  unsigned int di = abs(i - src_i);
  unsigned int dj = abs(j - src_j);
  double distance = cdm->distances_[di][dj];//根据查找表来获取到障碍物的距离
...
void map_update_cspace(map_t *map, double max_occ_dist)  //max_occ_dist通过参数设置的，表示障碍物点的最大膨胀距离
{
  unsigned char* marked;//己访问过列表
  std::priority_queue<CellData> Q;
  ...
  CachedDistanceMap* cdm = get_distance_map(map->scale, map->max_occ_dist);  //形成一个类似对数表的映射，相当于一次性计算完毕与最近占用栅格的距离
  ...
  while(!Q.empty())  //它不仅是单纯处理障碍物栅格，而是从近到远依次处理所有栅格与最近障碍物距离
  {
    CellData current_cell = Q.top();//上下左右的去遍历，BFS  //将距离障碍物最近的某个栅格的上下左右的栅格加入
    ...
    Q.pop(); //这里弹出的是加入上下左右后重新排序之后的最小数据的栅格
  }
